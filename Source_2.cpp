...
現在，動作させているのはDeAlgorithmNo==6
Bestの値の更新率と前回の個体との更新率でパラメータを分離を考える．

Schwefel functionで動作不良　最適解を超えた最小値をとる
		...
		for (DeAlgorithmNo = 6; DeAlgorithmNo <= 6; DeAlgorithmNo++) {
			...
			for (iteration = 0; iteration < EXTIME; iteration++) {
				...
				while (episode < MaxGrnration) {
					...
					episode++;
					if (Terminate > gBestFitness) break;	//終了条件を満たせば終了
				}
