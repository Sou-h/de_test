...
void QuickSort_hoge(double a[MAXGRNRATION][EXTIME], int left, int right,int Nom) {
	if (right - left <= 1) return;
	int pivot_index = (left + right) / 2;  // 適当にここでは中点とします
	int pivot = a[Nom][pivot_index];
	swap(&a[Nom][pivot_index], &a[Nom][right - 1]);    // pivot と右端を swap
	int i = left; // iterator
	for (int j = left; j < right - 1; ++j) { // j は全体を眺めて
		if (a[Nom][j] < pivot) { // pivot 未満のがあったら左に詰めていく
			swap(&a[Nom][i++], &a[Nom][j]);
		}
	}
	swap(&a[Nom][i], &a[Nom][right - 1]); // pivot を適切な場所に挿入
	// 再帰的に解く 
	QuickSort_hoge(a, left, i,Nom);    // 左半分 (pivot 未満)
	QuickSort_hoge(a, i + 1, right,Nom); // 右半分 (pivot 以上)
}

/*

void QuickSort(double a[][EXTIME], int left, int right,int Nom) {
	if (right - left <= 1) return;
	...
	swap(&a[Nom][i], &a[Nom][right - 1]); // pivot を適切な場所に挿入
	 再帰的に解く 
	QuickSort(a, left, i,Nom);    // 左半分 (pivot 未満)
	QuickSort(a, i + 1, right,Nom); // 右半分 (pivot 以上)
}
*/
...
void Parameter_Format(int iDe_nomber) {
	...
	else if (iDe_nomber == 8 || iDe_nomber == 7) {
		New_parameter_2();
